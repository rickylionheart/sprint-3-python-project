...
# Slice out the first 7 characters ("Pokemon")
pokemon_word = game_name[0:7]
print(pokemon_word)


# --- Part B: Clean Messy Strings ---
messy_names = ['  Wii Sports  ', 'TETRIS', '  mario kart WII']

for name in messy_names:
    # Strip edge spaces and convert to lowercase
    cleaned_name = name.strip().lower()
    print(cleaned_name)


# --- Part C: F-String Summary ---
# Grab the #1 game row (index 0)
top_game = video_game_sales[0]

# Construct and print the formatted summary
summary = f"#1 Best Seller: {top_game[NAME]} ({top_game[YEAR]}) - ${top_game[GLOBAL_SALES]}M global sales"
print(summary)
