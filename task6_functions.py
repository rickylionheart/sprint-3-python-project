...
def calculate_total_sales(game):
    # Sum up the sales at indices 6, 7, and 8
    total = game[6] + game[7] + game[8]
    return total

# Test it on the first game in the dataset
first_game_sales = calculate_total_sales(video_game_sales[0])
print(first_game_sales)
...
# Test 1: Without specifying a genre (defaults to 'Platform')
platform_games = filter_by_genre(video_game_sales)
print(f"Found {len(platform_games)} Platform games.")

# Test 2: Specifying a genre (e.g., 'Action')
action_games = filter_by_genre(video_game_sales, 'Action')
print(f"Found {len(action_games)} Action games.")
...
# Use the function inside a loop to print a summary of every game
